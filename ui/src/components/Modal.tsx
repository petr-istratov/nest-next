...
export interface ModalProps {
  close: () => void;
  buttons: { action: (event: any) => Promise<void> | void; title: string; primary?: boolean }[];
  body: () => ReactNode;
  title: string;
}
      ...
      <ModalCard pad={`10px`} background={`white`}>
        ...
        <CardFooter justify="center">
          {props.buttons.map((button, key) => (
            <Button key={key} primary={button.primary} label={button.title} onClick={() => void button.action(body)} />
          ))}
        </CardFooter>
      </ModalCard>
    </Hover>
